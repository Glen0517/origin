...
#define BD_ADDR_TYPE_LE_PUBLIC_IDENTITY   2
#define BD_ADDR_TYPE_LE_RANDOM_IDENTITY   3
...
} __attribute__((packed)) le_set_ext_adv_params_cp;

#define LE_SET_EXT_ADV_PARAMS_CP_SIZE 25

typedef struct {
	uint8_t  status;
	int8_t   selected_tx_power;
} __attribute__ ((packed)) le_set_ext_adv_params_rp;
#define LE_SET_EXT_ADV_PARAMS_RP_SIZE 9

#define OCF_LE_SET_EXT_ADV_DATA    0x0037
typedef struct {
	uint8_t  advertising_handle;     // identifies advertising set
	uint8_t  operation;              // complete, first fragment, intermediate fragment, last fragment
	uint8_t  fragment_preference;    // controller can fragment or not
	uint8_t  advertising_data_length; // length of advertising data (0 to 251)
	uint8_t  advertising_data[251];  // actual advertising data
} __attribute__((packed)) le_set_ext_adv_data_cp;

#define LE_SET_EXT_ADV_DATA_CP_MAX_SIZE  (4 + 251)

#define OCF_LE_SET_EXT_SCAN_RSP_DATA   0x0038
...
typedef struct {
	...
	uint8_t scan_response_data[31];  // using classic 31-byte scan rsp data length for compatibility
} __attribute__((packed)) le_set_ext_scan_rsp_data_cp;

#define LE_SET_EXT_SCAN_RSP_DATA_CP_SIZE (4 + 31)
...
} __attribute__((packed)) le_set_ext_adv_enable_cp;

#define LE_SET_EXT_ADV_ENABLE_CP_SIZE (2 + 4)

#define OCF_LE_SET_EXT_ADV_RANDOM_ADDRESS		0x0035
typedef struct {
	uint8_t  advertising_handle;
	bdaddr_t bdaddr;
} __attribute__ ((packed)) le_set_ext_adv_random_address_cp;

#define LE_SET_EXT_ADV_RANDOM_ADDRESS_CP_SIZE 7
