...
int network_music_init(void) {
    ...
    LOG_INFO("Network music service initialized successfully");
    return 0;
}

void network_music_deinit(void) {
    curl_global_cleanup();
    LOG_INFO("Network music service deinitialized");
}
...
int network_music_get_metadata(const char *url, char **metadata) {
    ...
    if(res != CURLE_OK) {
        LOG_ERROR("Failed to get metadata: %s", curl_easy_strerror(res));
    ...
    *metadata = chunk.response;
    curl_easy_cleanup(curl);
    LOG_INFO("Got metadata from URL: %s", url);
    return 0;
}
...
int network_music_stream(const char *url, void (*data_callback)(const char *, size_t)) {
    ...
    CURLcode res = curl_easy_perform(curl);
    if(res != CURLE_OK) {
        LOG_ERROR("Failed to stream music: %s", curl_easy_strerror(res));
        curl_easy_cleanup(curl);
        return -1;
    }

    curl_easy_cleanup(curl);
    LOG_INFO("Music streaming completed");
    return 0;
}
...
int network_music_search(const char *query, char **results) {
    ...
    CURLcode res = curl_easy_perform(curl);
    if(res != CURLE_OK) {
        LOG_ERROR("Failed to search music: %s", curl_easy_strerror(res));
        curl_easy_cleanup(curl);
        free(chunk.response);
        return -1;
    }
    ...
    *results = chunk.response;
    curl_easy_cleanup(curl);
    LOG_INFO("Searched for music: %s", query);
    return 0;
}